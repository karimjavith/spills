  ...
  return (
    <div className={modalStyles.backdrop} role="dialog" aria-modal="true">
      <div className={modalStyles.modalCard}>
        <span className={modalStyles.avatar}>👤</span>
        <div className={modalStyles.headerRow}>
          <span className={modalStyles.accountName}>Account name: {accountName || "Account Name"}</span>
          <span className={modalStyles.accountType}>Type: {accountType || "Personal"}</span>
        </div>
  ...
  <ul className={modalStyles.prefList}>
    <li className={modalStyles.prefItem}>
      <label className={modalStyles['toggle-switch']}>
        <input type="checkbox" checked disabled aria-label="Savings Goal" />
...
export default function ProfileIcon({ accountName, accountType }) {
  ...
  return (
    <>
      ...
      <ProfileModal open={open} onClose={() => setOpen(false)} accountName={accountName} accountType={accountType} />
    </>
  );
}
